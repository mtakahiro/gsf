...
FILTER array must correspond to response curve files in ``DIR_FILT``. For example, if one of the keywords in ``FILTER`` is "205", then gsf will look into DIR\_FILT directory to find a filter response file "205.fil", whose format should be in;
...
Spectral data
-------------
gsf reads the ascii spectral file for the target object, in ``DIR_EXTR`` in the configuration file. The file should be specified in ``SPEC_FILE``, whose formats are;
...
The parameters should be stored in an ascii file, ``MORP_FILE``, in the following format;
