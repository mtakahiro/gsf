# -*- coding: utf-8 -*-
# Some modification from corner plot package
#
...
def corner(xs, bins=20, range=None, weights=None, color="k",
    ...
    **hist2d_kwargs
    ...
    if fig is None:
        fig, axes = pl.subplots(K, K, figsize=(dim*sclfig, dim*sclfig))
    else:
        try:
    ...
    fig.subplots_adjust(left=lb* 1.08, bottom=lb, right=tr, top=tr*1.08,
    ...
    for i, x in enumerate(xs):
        ...
        if smooth1d is None:
            n, _, _ = ax.hist(x, bins=bins[i], weights=weights,
                              range=np.sort(range[i]), lw=2., **hist_kwargs)
        else:
            if gaussian_filter is None:
                raise ImportError("Please install scipy for smoothing")
            n, b = np.histogram(x, bins=bins[i], weights=weights,
                                range=np.sort(range[i]), lw=1.5)
        ...
        if len(quantiles) > 0:
            ...
            for q in qvalues:
                ax.axvline(q, ls="dashed", color='gray')
...
def hist2d(x, y, bins=20, range=None, weights=None, levels=None, smooth=None,
    ...
    if plot_contours:
        ...
        NL = len(levels)
        lw_cont = np.zeros(NL, dtype='float32')
        ii = 0
        for a in enumerate(levels):
            lw_cont[ii] = 0.3 + NL * 0.5 - 0.5 * ii
            ii +=1

        ax.contour(X2, Y2, H2.T, V, linewidths=lw_cont, **contour_kwargs)
...
def corner_TM(xs, bins=20, range=None, weights=None, color="k",
           smooth=None, smooth1d=None,
           labels=None, label_kwargs=None,
              show_titles=False, title_fmt=".2f", title_kwargs=None, titles=None,
           truths=None, truth_color="#4682b4",
           scale_hist=False, quantiles=None, verbose=False, fig=None,
           max_n_ticks=5, top_ticks=False, use_math_text=False,
            hist_kwargs=None, **hist2d_kwargs):
    ...
    # Some magic numbers for pretty axis layout.
    K = len(xs)
    factor = 1.2           # size of one side of one panel
    lbdim = 0.5 * factor   # size of left/bottom margin
    trdim = 0.2 * factor   # size of top/right margin
    whspace = 0.0         # w/hspace size
    plotdim = factor * K + factor * (K - 1.) * whspace
    dim = lbdim + plotdim + trdim
