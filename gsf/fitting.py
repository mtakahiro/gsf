...
class Mainbody(GsfBase):
    """Main SED-fitting class for the GSF package.

    The :class:`Mainbody` class provides routines to prepare templates, read
    observational data, configure fit parameters, run optimization and
    sampling (e.g. MCMC), and produce result plots and diagnostics.

    Parameters
    ----------
    inputs : dict
        Configuration dictionary or parsed input file with keys described in
        the package documentation.
    c : float, optional
        Speed of light in Angstrom/s (default: 3e18).

    Notes
    -----
    The class supports multiple samplers (EMCEE, ZEUS, Dynesty) and options
    for nebular emission, dust attenuation, AGN components, and redshift as a
    free parameter. Input fields are validated against expected configuration
    keywords.
    """
