...
###############
def get_evolv(ID0, PA, Z=np.arange(-1.2,0.4249,0.05), age=[0.01, 0.1, 0.3, 0.7, 1.0, 3.0], f_comp = 0, fil_path = './FILT/', inputs=None, dust_model=0, DIR_TMP='./templates/'):
    #
    ...
    # Input SFH
    #ax1.errorbar(Tuni/cc.Gyr_s-tuniv_hr, (sfh_hr_in), linestyle='--', color='r', lw=3., marker='')
    # Input ZH
    #ax1.errorbar(Tuni/cc.Gyr_s-tuniv_hr, np.log10(zh_hr_in), linestyle='-', color='b', lw=0.5, marker='')
    #plt.show()
    #print(hoge)
    ...
    sp.set_tabular_sfh(tuniv_hr[con_sfh], sfh_hr_in[con_sfh], zh_hr_in[con_sfh])
    ...
    for ss in range(len(t_get)):
        ...
        if ss == 0:
            ...
            col1   = fits.Column(name='wavelength', format='E', unit='AA', disp='obs', array=wavetmp)
            col2   = fits.Column(name='colnum', format='K', unit='', array=nd_ap)
            col00  = [col1, col2]

        colspec_all = fits.Column(name='fspec_'+str(ss), format='E', unit='Fnu', disp='%s'%(t_get[ss]), array=spec_mul_nu_conv[ss,:])
