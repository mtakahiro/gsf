...
def run_gsf_template(inputs, fplt=0, tau_lim=0.001, idman=None, nthin=1, delwave=10,
    ...
    if fplt == 0 or fplt == 1 or fplt == 2:
        ...
        if fplt==0:
            ...
            if MB.f_general:
                ...
                from .templates_z0 import make_templates_z0_tau
                make_templates_z0_tau(MB, lammax=lammax)            
    ...
    if fplt<2:
        ...
        #
        if MB.SFH_FORM == -99:
            make_templates(MB, tau_lim=tau_lim, nthin=nthin, delwave=delwave, f_IGM=f_IGM)
        else:
            make_templates(MB, tau_lim=tau_lim, nthin=nthin, delwave=delwave, f_IGM=f_IGM)
...
def run_gsf_all(parfile, fplt, cornerplot=True, f_plot_chain=True, f_Alog=True, idman:str=None, 
    ...
    if fplt <= 2:
        ...
        #
        if MB.SFH_FORM == -99:
            flag_suc = make_templates(MB, tau_lim=MB.tau_lim, nthin=MB.nthin, delwave=MB.delwave)
        else:
            flag_suc = make_templates(MB, tau_lim=MB.tau_lim, nthin=MB.nthin, delwave=MB.delwave)
    ...
    if fplt <= 3 and flag_suc:
        ...
        if not MB.ztemplate:
            if MB.SFH_FORM == -99:
                flag_suc = make_templates(MB, tau_lim=tau_lim, nthin=nthin, delwave=delwave)
            else:
                flag_suc = make_templates(MB, tau_lim=tau_lim, nthin=nthin, delwave=delwave)
            if not flag_suc:
                return False
        ...
        if not skip_sfh:
            if MB.SFH_FORM == -99:
                ...
                clsplot.plot_sed(fil_path=MB.DIR_FILT,
