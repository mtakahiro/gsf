...
def plot_sed_Z(ID0, PA, Z=np.arange(-1.2,0.4249,0.05), age=[0.01, 0.1, 0.3, 0.7, 1.0, 3.0],  f_Z_all=0, tau0=[0.1,0.2,0.3], flim=0.01, fil_path='./', figpdf=False):
    ...
    for jj in range(len(II0)):
        ...
        fu_t = filconv(ysum_wid,lmrest_wid,fu,lu)
        fv_t = filconv(ysum_wid,lmrest_wid,fv,lv)
        fj_t = filconv(ysum_wid,lmrest_wid,fj,lj)
    ...
    f140_cnv = filconv(ysum,x0,fjh,ljh)
    ...
    try:
        ...
        axicon.text(10, 25.0, '%s'%(ID0), color='w', fontsize=14)
        axicon.text(11, 25.0, '%s'%(ID0), color='w', fontsize=14)
        axicon.text(10, 26.0, '%s'%(ID0), color='w', fontsize=14)
...
def plot_sed_sim(ID0, PA, Z=np.arange(-1.2,0.4249,0.05), age=[0.01, 0.1, 0.3, 0.7, 1.0, 3.0],  f_Z_all=0, tau0=[0.1,0.2,0.3], flim=0.01, figpdf=False):
    ...
    for ii in range(len(II0)):
        if ii == 0:
            ...
            ysum_in, x0in   = fnc.tmp03(ID0, PA, Ain[ii], Avin, ii, Zin[ii], zbes, lib_all, tau0=tau0)

        else:
            y0, x0_tmp   = fnc.tmp03(ID0, PA, A50[ii], AAv[0], ii, Z50[ii], zbes, lib_all, tau0=tau0)
            y0p, x0p     = fnc.tmp03(ID0, PA, A50[ii], AAv[0], ii, Z50[ii], zbes, lib, tau0=tau0)

            ysump += y0p
            ysum  += y0

            y0_r_in, x0_tmp_in = fnc.tmp03(ID0, PA, Ain[ii], Avin, ii, Zin[ii], zbes, lib_all, tau0=tau0)
            ysum_in  += y0_r_in
        ...
        '''
        fu_t = filconv(ysum,lmrest,fu,lu)
        fv_t = filconv(ysum,lmrest,fv,lv)
        fj_t = filconv(ysum,lmrest,fj,lj)
        uvt  = -2.5*log10(fu_t/fv_t)
        vjt  = -2.5*log10(fv_t/fj_t)
        #print(age[ii], uvt, vjt)
        '''


    lmrest = x0/(1.+zbes)
    fu_cnv = filconv(ysum,lmrest,fu,lu)
    fv_cnv = filconv(ysum,lmrest,fv,lv)
    fj_cnv = filconv(ysum,lmrest,fj,lj)
    ...
    flw = open(ID0 + '_PA' + PA + '_mag.txt', 'w')
    flw.write('# ID PA flux_u flux_v flux_j flux_f140w\n')
    flw.write('%s %s %.5f %.5f %.5f %.5f %.5f\n'%(ID0, PA, fu_cnv, fv_cnv, fj_cnv, f140_cnv, SN))
    flw.close()
    ...
    # Main result
    #############
    ax1.plot(x0, ysum * c/ np.square(x0) / d, '-', lw=1, color='k', zorder=-1, label='Input')
    ax1.plot(x0, ysum * c/ np.square(x0) / d, '--', lw=1, color='gray', zorder=-1, label='$\chi^2/\\nu=%.2f$'%(chin)) # chin

    ymax = np.max([fybb[conbb]*c/np.square(xbb[conbb])/d] or ysum*c/np.square(x0)/d) * 1.7
    ...
    ax1.set_xlabel('Observed wavelength ($\mathrm{\AA}$)', fontsize=14)
    ax1.set_ylabel('Flux ($10^{-18}\mathrm{erg}/\mathrm{s}/\mathrm{cm}^{2}/\mathrm{\AA}$)', fontsize=13)

    ax1.set_xlim(2000, 88000)
    #ax1.set_xlim(12500, 16000)
    ax1.set_xscale('log')
    ax1.set_ylim(-0.05, ymax)
    ...
    from mpl_toolkits.axes_grid1.inset_locator import mark_inset
    from mpl_toolkits.axes_grid.inset_locator import inset_axes
    ax2t = inset_axes(ax1, width="30%", height="30%", loc=2)
    ...
    # Zoom Line
    ############
    ax2t.plot(x0in/zscl, ysum_in * c/np.square(x0in)/d, '-', lw=0.5, color='k', zorder=3.)
    ax2t.plot(x0/zscl, ysum * c/np.square(x0)/d, '--', lw=0.5, color='gray', zorder=3.)
    ...
    #######################################
    ax2t.set_xlabel('Restframe wavelength ($\mathrm{\AA}$)')
    ax2t.set_xlim(3600, 5400)
    conaa = (x0/zscl>3300) & (x0/zscl<6000)
    ymaxzoom = np.max(ysum[conaa]*c/np.square(x0[conaa])/d) * 1.2
    yminzoom = np.min(ysum[conaa]*c/np.square(x0[conaa])/d) / 1.2
    ...
    # Line name
    ###############
    LN = ['Mg2', '$NeIV$', '[OII]', 'H$\theta$', 'H$\eta$', 'Ne3?', 'H$\delta$', 'H$\gamma$', 'H$\\beta$', 'O3', 'O3', 'Mgb', 'Halpha', 'S2L', 'S2H']
    LW = [2800, 3347, 3727, 3799, 3836, 3869, 4102, 4341, 4861, 4959, 5007, 5175, 6563, 6717, 6731]

    fsl = 9 # Fontsize for line
    for ll in range(len(LN)):
        if ll == 2 and FLW[ll] == 1:
            yyl = np.arange(yminzoom+(ymaxzoom-yminzoom)*0.5,yminzoom+(ymaxzoom-yminzoom)*0.65, 0.01)
            xxl = yyl * 0 + LW[ll]
            ax2t.errorbar(xxl, yyl, lw=0.5, color=lcb, zorder=20, alpha=1., label='', capsize=0)
            ax2t.text(xxl[0]-130, yyl[0]*1.28, '%s'%(LN[ll]),  color=lcb, fontsize=9, rotation=90)

        elif (ll == 9 and FLW[ll] == 1):
            yyl = np.arange(yminzoom+(ymaxzoom-yminzoom)*0.5,yminzoom+(ymaxzoom-yminzoom)*0.65, 0.01)
            xxl = yyl * 0 + LW[ll]
            ax2t.errorbar(xxl, yyl, lw=0.5, color=lcb, zorder=20, alpha=1., label='', capsize=0)
            #ax2t.text(xxl[0]+40, yyl[0]*1.1, '%s'%(LN[ll]),  color='royalblue', fontsize=7, rotation=90)

        elif (ll == 10 and FLW[ll] == 1):
            yyl = np.arange(yminzoom+(ymaxzoom-yminzoom)*0.5,yminzoom+(ymaxzoom-yminzoom)*0.65, 0.01)
            xxl = yyl * 0 + LW[ll]
            ax2t.errorbar(xxl, yyl, lw=0.5, color=lcb, zorder=20, alpha=1., label='', capsize=0)
            ax2t.text(xxl[0]+40, yyl[0]*1.15, '%s'%(LN[ll]),  color=lcb, fontsize=9, rotation=90)

        elif FLW[ll] == 1 and (ll == 6 or ll == 7 or ll == 8):
            yyl = np.arange(yminzoom+(ymaxzoom-yminzoom)*0.2,yminzoom+(ymaxzoom-yminzoom)*0.35, 0.01)
            xxl = yyl * 0 + LW[ll]
            ax2t.errorbar(xxl, yyl, lw=0.5, color=lcb, zorder=20, alpha=1., label='', capsize=0)
            ax2t.text(xxl[0]+40, yyl[0]*0.95, '%s'%(LN[ll]),  color=lcb, fontsize=9, rotation=90)

        elif ll == 6 or ll == 7 or ll == 8:
            yyl = np.arange(yminzoom+(ymaxzoom-yminzoom)*0.2,yminzoom+(ymaxzoom-yminzoom)*0.35, 0.01)
            xxl = yyl * 0 + LW[ll]
            ax2t.errorbar(xxl, yyl, lw=0.5, color='gray', zorder=1, alpha=1., label='', capsize=0)
            ax2t.text(xxl[0]+40, yyl[0]*0.95, '%s'%(LN[ll]),  color='gray', fontsize=9, rotation=90)

        elif FLW[ll] == 1:
            yyl = np.arange(yminzoom+(ymaxzoom-yminzoom)*0.7,yminzoom+(ymaxzoom-yminzoom)*.95, 0.01)
            xxl = yyl * 0 + LW[ll]
            ax2t.errorbar(xxl, yyl, lw=0.5, color=lcb, zorder=20, alpha=1., label='', capsize=0)
            ax2t.text(xxl[0]+40, yyl[0]*1.25, '%s'%(LN[ll]),  color=lcb, fontsize=9, rotation=90)
    ...
    ax1.legend(loc=1, fontsize=11)
    if figpdf:
        plt.savefig('SPEC_' + ID0 + '_PA' + PA + '_sim.pdf')
    else:
        plt.savefig('SPEC_' + ID0 + '_PA' + PA + '_sim.png', dpi=200)
